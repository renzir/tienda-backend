const OrderService = require("./order.service"); // O donde hayas guardado el service unificado
...
class OrderController {
  ...
  static async getOrders(req, res, next) {
    try {
      ...
      // Si hay un ID en la URL, buscamos esa orden específica
      if (id) {
        const orderDetails = await OrderService.getOrderDetails(id);

        if (!orderDetails) {
          // Lanzar AppError para que el middleware global maneje el 404
      ...
      // Si NO hay ID, listamos todas las órdenes.
      // Aquí es donde solía fallar si se pasaba un ID vacío o nulo al service de detalles
  ...
  static async createOrder(req, res, next) {
    try {
      const { usuario_id, direccion } = req.body; // <--- Cambiado
  ...
  static async addProduct(req, res, next) {
    try {
      const { id, productId } = req.params; // Capturamos ambos parámetros
      const { cantidad, precio } = req.body;
      // Validación de integridad antes de tocar el servicio
  ...
  static async removeProduct(req, res, next) {
    try {
      const { id: orderId, productId } = req.params; // Asumiendo URL como /orders/:id/remove-product
